...
from util.constants import Domain
...
# def chat_conditional(chat: bool) -> str:
#     return "chat" if chat else "text"
...
def load_json(base_dir: str, input_filename: str) -> list:
    """
    loads json file
    """
    path = os.path.join(base_dir, input_filename)
    return json.load(open(f"{path}.json", "r"))


def save_json(response: Dict[any, any], base_dir: str, output_filename: str) -> None:
    """
    saves json file
    """
    path = os.path.join(base_dir, output_filename)
    json.dump(
        response,
        open(
            f"{path}.json",
